...
export default function TextInput({ className = "", hasError, ...props }: TextInputProps) {
  return <input aria-invalid={hasError || undefined} onInvalid={localizeValidity} onInput={clearValidity} className={`${baseClassName} ${hasError ? errorClassName : ""} ${className}`.trim()} {...props} />;
}
