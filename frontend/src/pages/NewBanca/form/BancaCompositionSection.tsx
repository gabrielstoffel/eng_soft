...
const ROLES_BY_TAB: Record<CompositionTab, readonly (typeof ALL_ROLES)[number][]> = {
...
export default function BancaCompositionSection({ disabled = false }: BancaCompositionSectionProps) {
  ...
  const visibleRolesByTab = useMemo(
    () => ({
      orientacao: ROLES_BY_TAB.orientacao.filter((role) => visibleRoles[role] !== "hidden"),
      internos: ROLES_BY_TAB.internos.filter((role) => visibleRoles[role] !== "hidden"),
      externos: ROLES_BY_TAB.externos.filter((role) => visibleRoles[role] !== "hidden"),
    }),
    [visibleRoles],
  );
  ...
  const activeRolesByTab = useMemo(
    () => ({
      orientacao: activeRoles.filter((role) => ROLES_BY_TAB.orientacao.includes(role)),
      internos: activeRoles.filter((role) => ROLES_BY_TAB.internos.includes(role)),
      externos: activeRoles.filter((role) => ROLES_BY_TAB.externos.includes(role)),
    }),
    [activeRoles],
  );
  ...
  const optionalRolesByTab = useMemo(
    () => ({
      orientacao: ROLES_BY_TAB.orientacao.filter((role): role is OptionalMemberRole => visibleRoles[role] === "optional"),
      internos: ROLES_BY_TAB.internos.filter((role): role is OptionalMemberRole => visibleRoles[role] === "optional"),
      externos: ROLES_BY_TAB.externos.filter((role): role is OptionalMemberRole => visibleRoles[role] === "optional"),
    }),
    [visibleRoles],
  );
  ...
  return (
    <section className="overflow-hidden rounded-xl bg-white shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)]">
      <div className="border-b border-slate-200 px-4 py-5 sm:px-8">
        <div className="text-xs font-semibold tracking-[0.18em] text-sky-700 uppercase">Etapa 2 de 2</div>
        <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">Composição da banca</h2>
      </div>

      <div className="space-y-6 px-4 py-6 sm:px-8 sm:py-8">
        <div>
          <div className="flex items-center gap-1">
            <IconUsersGroup aria-hidden="true" size={18} stroke={1.9} className="shrink-0 self-center text-sky-700" />
            <h3 className="text-sm font-semibold tracking-[0.12em] text-slate-900 uppercase">Participantes</h3>
          </div>
          <p className="mt-2 text-sm leading-6 text-slate-600">Informe os participantes da banca e adicione os opcionais quando necessário.</p>
        </div>

        {availableTabs.length > 1 && (
          <div className="rounded-xl bg-slate-100 p-1">
            <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${availableTabs.length}, minmax(0, 1fr))` }}>
        ...
        <div className="space-y-3">
          ...
          {optionalRolesByTab[activeTab].some((role) => watch(role) === null) && (
            <div className="flex flex-wrap gap-2 pt-2">
              {optionalRolesByTab[activeTab].map((role) =>
                watch(role) === null ? (
                  ...
                  >
                    <IconPlus aria-hidden="true" size={15} stroke={2} className="shrink-0" />
