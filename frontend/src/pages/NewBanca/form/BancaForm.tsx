...
import BancaAttachmentsSection from "./BancaAttachmentsSection";
import {
  emptyMemberForm,
...
export function getInitialForm(ppg: Ppg): NewBancaFormState {
...
export function deserializeBanca(req: BancaRequest): NewBancaFormState {
  const fillMember = (m: MemberInfo | null) =>
    m
      ? {
          ...
          ...m,
          email: m.email ?? "",
          remoto: m.remoto ?? false,
          ppg: m.ppg ?? "",
          bolsista_cnpq: m.bolsista_cnpq ?? false,
          nivel_cnpq: m.nivel_cnpq ?? "",
          lattes: m.lattes ?? "",
          doctorate_institution: m.doctorate_institution ?? "",
          doctorate_year: m.doctorate_year ? String(m.doctorate_year) : "",
        }
      : null;
  ...
  return {
    ppg: req.ppg,
    nome: {
      gender: req.nome.gender,
      name: req.nome.name,
      cpf: req.nome.cpf ?? "",
      birth_date: req.nome.birth_date ?? "",
      email: req.nome.email ?? "",
    },
    tipo: req.tipo,
    data: req.data || "",
    horario: req.horario || "",
    modalidade: req.modalidade,
    sala_preferencia: req.sala_preferencia || "",
    link: req.link || "",
    titulo: req.titulo,
    titulo2: req.titulo2 || "",
    comentario_desempenho: req.comentario_desempenho || "",
    justificativa_membros: req.justificativa_membros || "",
...
export default function BancaForm({
  ...
  useEffect(() => {
    if (submittedSuccessfully) setStep(1);
      ...
      {step === 1 ? (
        <>
          ...
          <div className="flex justify-end">
            <button type="button" onClick={goToComposition} disabled={disabled} className="inline-flex w-full cursor-pointer items-center justify-center rounded-xl bg-sky-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:bg-slate-300">
              Continuar
            </button>
          </div>
        </>
        ...
        <>
          ...
          <BancaAttachmentsSection disabled={disabled} />
          <div className="flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={() => setStep(1)} disabled={loading} className="inline-flex w-full cursor-pointer items-center justify-center rounded-xl border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:bg-slate-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400 sm:w-1/2">
              Voltar
            </button>
            <button type="submit" disabled={loading} className="inline-flex w-full cursor-pointer items-center justify-center rounded-xl bg-sky-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:bg-slate-300 sm:w-1/2">
